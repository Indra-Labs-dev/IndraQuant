...
async def test_compute_returns_expected_fields():
    closes, volumes = _series()
    service = FeatureStoreService(cache=None)
    vector = await service.get_latest(1, "1h", _AS_OF, closes, volumes)
...
async def test_returns_none_fields_with_insufficient_history():
...
async def test_cache_hit_skips_recomputation():
    ...
    first = await service.get_latest(1, "1h", _AS_OF, closes, volumes)
    assert cache.sets == 1
    ...
    second = await service.get_latest(1, "1h", _AS_OF, [1.0, 2.0, 3.0], [1.0, 2.0, 3.0])
    assert second == first
    assert cache.sets == 1


async def test_different_as_of_misses_cache():
    closes, volumes = _series()
    cache = FakeCache()
    service = FeatureStoreService(cache=cache)

    await service.get_latest(1, "1h", _AS_OF, closes, volumes)
    later = datetime(2026, 1, 1, 1, tzinfo=timezone.utc)
    await service.get_latest(1, "1h", later, closes, volumes)
