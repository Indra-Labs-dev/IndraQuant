...
async def test_second_call_within_cooldown_skips_provider_fetch():
    ...
    await use_case.execute(1, "1h", datetime(2026, 1, 1), datetime(2026, 1, 2), 500)
...
async def test_cooldown_is_scoped_per_instrument_and_timeframe():
    ...
    await GetOhlcvUseCase(
        FakeInstruments([BTC]), provider_a, store_a, EventBus(), cache=cache
    ).execute(1, "1h", datetime(2026, 1, 1), datetime(2026, 1, 2), 500)
    await GetOhlcvUseCase(
        FakeInstruments([BTC]), provider_b, store_b, EventBus(), cache=cache
    ).execute(1, "5m", datetime(2026, 1, 1), datetime(2026, 1, 2), 500)

    assert len(provider_a.calls) == 1
    assert len(provider_b.calls) == 1


async def test_no_cache_never_debounces():
    store = FakeStore([candle(datetime(2026, 1, 1, 0))])
    provider = FakeProvider(
        [[candle(datetime(2026, 1, 1, h)) for h in (0, 1)]]
        + [[candle(datetime(2026, 1, 1, h)) for h in (0, 1)]]
